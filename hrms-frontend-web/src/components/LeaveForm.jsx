  ...
  const handleSegmentChange = (index, e) => {
    ...
    setLeaveSegments((prev) => {
      ...
      if (name === `fromDuration-${index}`) {
        ...
        newSegments[index] = {
          ...newSegments[index],
          dates: { ...newSegments[index].dates, fromSession: value },
        };
        ...
        newSegments[index] = {
          ...
          dates: {
            ...newSegments[index].dates,
            toDuration: value,
            toSession: value === "half" ? newSegments[index].dates.toSession || "forenoon" : "",
          },
        };
...
const handleSubmit = async (e) => {
  ...
  leaveSegments.forEach((segment, index) => {
    // Append all fields explicitly to avoid missing keys
  ...
  const getMaxDateForToDate = (index) => {
    ...
    const remainingDays = 3 - totalLeaveDays + calculateLeaveDays(segment);
    let maxLimit = new Date(fromDate);
    let daysAdded = 0;
    while (daysAdded < remainingDays - 1) {
  ...
  const tileDisabled = ({ date, view }) => {
    if (view === "month") {
      ...
      const maxDateForSegment = getMaxDateForToDate(0);
      return (
        date < minDateForSegment ||
        date > maxDateBase ||
        (leaveSegments[0].dates.to && date > maxDateForSegment)
      );
    }
    return false;
  };
                                ...
                                <div className="flex items-center gap-4">
                                  <label className="flex items-center">
                                    <input
                                      ...
                                      onChange={(e) => handleSegmentChange(index, e)}
                                      className="mr-2"
                                      disabled={getTotalLeaveDays() <= 0.5}
                                    />
                                    Full Day
                                  </label>
                                  <label className="flex items-center">
                          ...
                          <div className="col-span-2">
                            ...
                            <div className="flex items-center gap-6">
                              <Input
                                ...
                                disabled={segment.dates.fromDuration === "half" && segment.dates.fromSession === "forenoon"}
                              />
                        ...
                        {segment.leaveType === "Restricted Holidays" && (
                          <div className="col-span-2">
                            ...
                            >
                              ...
                              <SelectContent>
                                {restrictedHolidayDates.map((holiday) => (
                                  <SelectItem key={holiday.label} value={holiday.label}>
                                    {holiday.label} - {holiday.toLocaleDateString()}
                                  </SelectItem>
                                ))}
                              </SelectContent>
                            </Select>
          ...
          <Card className="shadow-lg border w-full">
            <CardContent className="p-6">
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="text-sm text-blue-800 flex items-center gap-4">
                  <div>
                  ...
                  <tbody>
                    {leaveSegments.map((segment, index) => (
                      <tr key={index}>
                        ...
                        <td className="border p-2">{segment.leaveType || "Not selected"}</td>
                        <td className="border p-2">{segment.dates.from ? formatDate(segment.dates.from) : "Not selected"}</td>
                        <td className="border p-2">{segment.dates.to ? formatDate(segment.dates.to) : "Not selected"}</td>
